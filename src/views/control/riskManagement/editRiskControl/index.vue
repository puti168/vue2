...
export default {
  ...
  computed: {
    ...
    content() {
      const obj = ['UserName', 'AgentName', 'BankCard', 'Virtual', 'Ip', 'Device']
    ...
    rules() {
      ...
      const userName = [{ required: true, message: '请输入会员账号', trigger: 'blur' }]
      const proxyUserName = [
        { required: true, message: '请输入代理账号', trigger: 'blur' }
      ]
      const cardNumber = [{ required: true, message: '请输入银行卡号', trigger: 'blur' }]
      const virtualAddress = [
        { required: true, message: '请输入虚拟币地址', trigger: 'blur' }
      ]
      const registerIp = [{ required: true, message: '请输入IP', trigger: 'blur' }]
      const deviceNo = [{ required: true, message: '请输入终端设备号', trigger: 'blur' }]
      const windReason = [{ required: true, message: '请输入审核信息', trigger: 'blur' }]
      return {
        windControlType,
        windControlName,
        userName,
  ...
  methods: {
    ...
    getMerchantDict(val) {
      ...
      this.$api.getSelectWindControlLevel({ windControlType: val * 1 }).then((res) => {
      ...
      switch (type) {
        case 'userName': {
          ...
          if (userName) {
            ...
            this.queryInfoData({ objectInfo: userName }, windControlType).then((res) => {
              const { id } = res
              if (id || id + '' === '0') {
                this.tipsShow = null
                this.current = 0
        ...
        case 'proxyUserName': {
          ...
          if (proxyUserName) {
            this.$refs.form.clearValidate('windControlName')
            this.$refs.form.clearValidate('windReason')
            this.queryInfoData({ objectInfo: proxyUserName }, windControlType).then(
              (res) => {
                const { id } = res
                if (id || id + '' === '0') {
                  this.tipsShow = null
        ...
        case 'cardNumber': {
          ...
          if (cardNumber) {
            this.$refs.form.clearValidate('windControlName')
            this.$refs.form.clearValidate('windReason')
            this.queryInfoData({ objectInfo: cardNumber }, windControlType).then(
              (res) => {
                const { id } = res
                if (id || id + '' === '0') {
                  this.tipsShow = null
        ...
        case 'registerIp': {
          ...
          if (registerIp) {
            this.$refs.form.clearValidate('windControlName')
            this.$refs.form.clearValidate('windReason')
            this.queryInfoData({ objectInfo: registerIp }, windControlType).then(
              (res) => {
                const { id } = res
                this.tipsShow = null
                if (id || id + '' === '0') {
