...
export default {
	...
	methods: {
		...
		handCheck(checkedNodes, checkedKeys) {
			this.menuIdsArr = checkedKeys.checkedKeys.concat(
			...
			if (
				this.menuIdsArr.length > 0 &&
				this.maxLength > this.menuIdsArr.length
			) {
				this.checkedAll = false
				this.isIndeterminate = true
			...
			this.$refs.form.validate((valid) => {
				if (valid) {
					if (!this.control) {
						...
						this.$api
							.setUpdateRoleInfo(this.editData)
							.then((res) => {
								if (res.code === 200) {
									this.$message({
										type: 'success',
