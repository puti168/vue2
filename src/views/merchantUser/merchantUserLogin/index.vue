				...
				>
					...
					>
						<template slot-scope="scope">
							<template v-for="(item,index) in scope.row.loginCountryList">
							<span v-if="index < 5" :key="item.countryName" class="area">
								{{ item.countryName }}({{ item.loginNumber }})
							</span>
							</template>
						</template>
					</el-table-column>
...
import { agentLoginRegion, listAll } from '@/api/waterSign'
...
export default {
	...
	methods: {
		...
		showDetail(item) {
			...
			this.list2 = item.loginCountryList
