				...
				>
					...
					>
						<template slot-scope="scope">
							...
							<el-button
								v-if="hasPermission('315') && scope.row.status === 0"
								type="danger"
								icon="el-icon-delete"
								size="medium"
				...
				<div class="content-part2">
					...
					>
						...
						<el-form-item
							...
							prop="discountTagId"
							:rules="[
								{ required: true, message: '请选择活动页签', trigger: 'change' }
							]"
						>
							<el-select
						...
						<el-form-item
							...
							prop="activityWashCodeTimes"
							:rules="[
								{ required: true, message: '请输入洗码倍率', trigger: 'blur' }
							]"
						>
							<el-input-number
		...
		>
			<table class="nativeTable">
				...
				<tr>
					...
					<td class="last">
						<div v-if="activityAreaSorting.length <= 0" class="nullData">
							暂无数据
						</div>
						<draggable
							v-else
...
export default {
	...
	methods: {
		...
		subSort() {
			...
			console.log(sortIds)
			this.$api
				.setOperateDiscountActivitySort({ sortIds: sortIds })
				.then((res) => {
					if (res.code === 200) {
						this.$message({
							message: '操作成功！',
		...
		subAddOrEdit() {
			...
			if (this.dialogForm.activityPrescription === '0') {
				this.dialogForm.activityPictureUpAt = dayjs(
					this.activityPictureUpAt
				).format('YYYY-MM-DD HH:mm:ss')
				this.dialogForm.activityPictureDownAt = dayjs(
					this.activityPictureDownAt
				).format('YYYY-MM-DD HH:mm:ss')
		...
		deleteBtn(row) {
			this.$confirm(
				...
				.then(() => {
					this.$api
						.setOperateDiscountActivityDelete({ id: row.id })
						.then((res) => {
							if (res.code === 200) {
								this.$message({
									message: '操作成功！',
